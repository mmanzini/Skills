...
name: notebooklm
description: >
  Use this skill whenever the user wants to generate an infographic, create a podcast or audio overview,
  or do any NotebookLM automation. Trigger on phrases like "make an infographic about X",
  "create a podcast from these sources", "turn this into an audio overview", "visualize this as an infographic",
  "generate a podcast episode", "add these URLs to NotebookLM", "create a deep-dive podcast",
  "I want an infographic of this research", "make a brief podcast", or any mention of NotebookLM.
  Also trigger for "/notebooklm" and any intent to summarize, research, or generate content from documents/URLs.
---
...
**Primary use cases (always trigger):**
- Infographic: "make an infographic", "visualize this", "create a visual summary", "infographic from these sources"
- Podcast: "create a podcast", "audio overview", "deep-dive podcast", "turn this into audio", "brief podcast about X"
...
**Primary generation types (main use cases):**
...
### Research to Infographic (Interactive)
**Time:** 5-15 minutes total (infographic generation may hit rate limits)
...
4. `notebooklm generate infographic --orientation landscape --detail standard` (confirm when asked)
   - Orientations: `landscape` (wide, default), `portrait` (tall), `square`
   - Detail levels: `concise` (less text), `standard`, `detailed` (more content)
5. Note the artifact ID returned
6. Check `notebooklm artifact list` later for status (infographic can take 5-15 min)
7. `notebooklm download infographic ./infographic.png` when complete (confirm when asked)

**Note:** Infographic generation is rate-limited. If it fails, wait 5-10 min and retry. Use the subagent pattern below for fully automated delivery.

### Research to Infographic (Automated with Subagent)
**Time:** 5-15 minutes, continues in background

1. Create notebook and add sources as usual
2. Wait for sources to be ready
3. Run `notebooklm generate infographic --orientation landscape --json` → parse `artifact_id`
4. **Spawn a background agent** using Task tool:
   ...
   Task(
     prompt="Wait for artifact {artifact_id} in notebook {notebook_id} to complete, then download.
             Use: notebooklm artifact wait {artifact_id} -n {notebook_id} --timeout 900
             Then: notebooklm download infographic ./infographic.png -a {artifact_id} -n {notebook_id}
             If exit code 2 (timeout): report timeout and suggest checking 'artifact list'.",
